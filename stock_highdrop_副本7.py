...
class StockAnalyzerApp:
    ...
    def analyze_stock(self, stock, begin_date=None, end_date=None):
        ...
        # 计算每段周期的最低值及下降百分比
        period_data = []
        decline_percentages = []  # 用于存储所有回调百分比
        for i in range(len(historic_high_dates) - 1):
            ...
            decline_percentages.append(decline_percentage)
        ...
        if last_high_date < end_date:
            ...
            decline_percentages.append(final_decline_percentage)
        ...
        # 计算回调中位数和平均数并输出
        if decline_percentages:
            median_decline = pd.Series(decline_percentages).median()
            mean_decline = pd.Series(decline_percentages).mean()
            print(f"\n回调中位数: {median_decline:.2f}%")
            print(f"回调平均数: {mean_decline:.2f}%")
