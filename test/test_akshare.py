        ...
        for code, name in stocks.items():
            for period_name, period in test_periods.items():
                for range_name, (start_date, end_date) in test_ranges.items():
                    try:
                        logger.info(f"测试 {market} {code} {name} {period_name} {range_name}")
                        ...
                        if not df.empty:
                            ...
                            filename = f"{code}_{period_name}_{range_name}.csv"
                            df.to_csv(os.path.join(save_path, filename))
                            
                    except Exception as e:
                        logger.error(f"测试失败: {market} {code} {period_name} {range_name} - {str(e)}")
                        results.append({
                            "market": market,
                            "code": code,
                            "name": name,
