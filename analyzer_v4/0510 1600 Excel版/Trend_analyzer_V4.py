...
def add_signals_to_docx(doc, signals, title):
    ...
    if not signals:
        doc.add_paragraph(f"{title} 无背离")
        return

    # 找到最后一个顶背离和最后一个底背离的时间
    top_indices = [i for i, s in enumerate(signals) if '顶背离' in s[1]]
    bottom_indices = [i for i, s in enumerate(signals) if '底背离' in s[1]]
    last_top_idx = top_indices[-1] if top_indices else None
    last_bottom_idx = bottom_indices[-1] if bottom_indices else None

    for idx, (t, s_type) in enumerate(signals):
        p = doc.add_paragraph()
        run = p.add_run(f"{t} - {s_type}")
        if idx == last_top_idx:
            run.font.color.rgb = RGBColor(0xFF, 0x00, 0x00)  # 红色
        elif idx == last_bottom_idx:
