...
def test_us_min_data():
    ...
    # 测试股票列表，直接加上市场代码前缀
    symbols = {
        'TSM': '106.TSM',   # TSM使用106前缀
        'MSFT': '105.MSFT'  # MSFT使用105前缀
    }
    ...
    for symbol_name, full_symbol in symbols.items():
        logger.info(f"\n开始获取 {symbol_name} 的1分钟数据")
        logger.info(f"使用完整代码: {full_symbol}")
        
        df = get_us_min_data(
            symbol=full_symbol,  # 直接使用带前缀的完整代码
            period="1min",
        ...
        if df is not None:
            logger.info(f"成功获取 {symbol_name} 的1分钟数据")
        else:
            logger.error(f"获取 {symbol_name} 的1分钟数据失败")
