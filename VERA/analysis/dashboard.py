...
def generate_dashboard_data(
    ...
    if risk_card:
        path_card_data = {
            ...
            # raw state
            "state": risk_metrics.get("risk_state", {}).get("state"),
            "has_new_high": risk_metrics.get("risk_state", {}).get("has_new_high", False),
        }
    ...
    if risk_card:
        ...
        current_dd = None
        if progress is not None and max_dd is not None:
            current_dd = progress * max_dd

        raw_pos = {
            # Use price_percentile for position logic (v2)
            "price_percentile": progress,
            "progress": progress, 
            "drawdown": current_dd, # Now explicitly Current Drawdown
        }
    ...
    return DashboardData(
        ...
        value={
            "valuation_status": getattr(fundamentals, "current_valuation_status", None),
            ...
            "current_pe": fundamentals.pe_ttm,
            "current_pe_static": fundamentals.pe_static,
            "eps_ttm": fundamentals.eps_ttm,
            "current_pb": fundamentals.pb_ratio,
            "dividend_yield": fundamentals.dividend_yield,
            "buyback_ratio": fundamentals.buyback_ratio,
            "pe_percentile": pe_percentile
        },
        overlay={**bank_data, **(overlay or {})},
        quality=quality_data,  # NEW: Add quality buffer data
