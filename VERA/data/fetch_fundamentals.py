...
def fetch_fundamentals(symbol: str, as_of_date: datetime = None) -> tuple[AssetFundamentals, BankMetrics]:
    ...
    if fin_row:
        # Unpack with new currency column
        eps, bps, rev, profit, div_amt, buy_amt, npl_r, prov_c, sm_r, report_currency = fin_row
        ...
        fundamentals = AssetFundamentals(
            ...
            bps=bps,
            eps_ttm=eps,
            current_valuation_status=val_status,
            revenue_history=revenue_history,  # NEW: Multi-year revenue for quality assessment
            no_dividend_history=no_dividend_history,
            listing_years=listing_years,
            npl_deviation=npl_r * 1.5 if npl_r else (1.0 if industry == "Bank" else None), # Simplified proxy
        ...
        if industry == "Bank":
            bank_metrics = BankMetrics(
                overdue_90_loans=npl_r * 1.5 * 1e8 if npl_r else 0.0,
                npl_balance=1e8 if npl_r else 0.0,
